...
var len = 50;
var clickSound = new Audio("media/click_char.mp3");
var changeSound = new Audio("media/char_changed.mp3");
var invalidSound = new Audio("media/enter_invalid.mp3");
...
function createPuzzles(div_id, message, key) {
	...
	if (key == undefined) {
		...
		for (var i = 0; i < codes.length; i++) {
			...
			for(x=i;x<codes[i].length;x++){
				temp = codes[i].charCodeAt(x);
				if((temp >= 65 && temp <= 90) || (temp >= 97 && temp <= 122))
					{
						noLetter = false;
						break;
			...
			value[k++] = codes[i];
				...
				for(x=temp.length-1;x>=0;x--){
					breaker = temp.charCodeAt(x);
					if (!((breaker >= 65 && breaker <= 90) || (breaker >= 97 && breaker <= 122))) {
						value[k++] =  temp.substr(0, x);
						temp = temp.substr(x);
						break;
					}
