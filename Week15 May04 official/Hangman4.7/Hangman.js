...
function callFunction(div_id) {
	...
	if (data.num <= count) {
		if(data.num==1){
			createPuzzles(div_id, data.question[data.quesDisplay[data.num - 1]],
					data.answer[data.quesDisplay[data.num - 1]], data.num)
		}
		else{
			disable(div_id);
			setTimeout(function(){createPuzzles(div_id, data.question[data.quesDisplay[data.num - 1]],
					data.answer[data.quesDisplay[data.num - 1]], data.num)},2000);}
...
function a(div_id) {
	...
	var timeInfo = showTime(data.startTime.getTime(), data.endTime
			.getTime(), div_id);
...
function replaceChar(outerDiv){
	var Hdata = map[outerDiv];
	if (Hdata.startTime == null) Hdata.startTime = new Date();
	...
	if((str.charCodeAt(0) >= 48 && str.charCodeAt(0) <= 57) ||(str.charCodeAt(0) >= 65 && str.charCodeAt(0) <= 90) ){
