...
function CryptogramData(div_id) {
	...
	this.solutionDiv = createSolutionDiv(div_id);
	this.hoverrevertclass = 'labelcolor1';
}

function createSolutionDiv(div_id){
	...
	for (var i = 0; i < encrypt.length; i++) {
		...
		for (var j = 0; j < encrypt[i].length; j++) {
			...
			.charCodeAt(0) <= 122)) {
						...
						for (var j = 0; j < encrypt[i].length; j++) {
							if (encrypt[i][j] == str
								...
								var a = document.getElementById(outerDiv + "lbAnswerItm"
										+ i + j);
								var crData1 = map[outerDiv];
								crData1.hoverrevertclass = q.className;
								map[outerDiv] = crData1;
								q.className = 'labelcolor3';
								a.className = 'labelcolor3';
							}
						}
					}
				}
						...
						for (var j = 0; j < encrypt[i].length; j++) {
							if (encrypt[i][j] == str
								...
								var a = document.getElementById(outerDiv + "lbAnswerItm"
										+ i + j);
								var crData1 = map[outerDiv];
								q.className = crData1.hoverrevertclass;
								a.className = crData1.hoverrevertclass;
							}
						}
					}
				}
				thelabel.onclick = function() {
					...
					var textbox = crData.solutionDiv.getElementsByTagName("input")[0];
					var curSolveBoxLine = this.parentNode;
					curSolveBoxLine.appendChild(crData.solutionDiv);
					crData.solutionSubLineId=curSolveBoxLine.id;
					...
					var crData1 = map[outerDiv];
					crData1.hoverrevertclass = 'labelcolor2';
					//alert(crData1.hoverrevertclass);
					map[outerDiv] = crData1;
		...
		for (var j = 0; j < encrypt[i].length; j++) {
			...
			.charCodeAt(0) <= 122)) {
				...
				thelabelAnswer.className = 'labelcolor1';
				thelabelAnswer.onmouseover = function() {
					var indices = this.value.split("-");
					var str = encrypt[indices[0]][indices[1]];
					var str2 = "";
					if (str.charCodeAt(0) >= 65 && str.charCodeAt(0) <= 90) {
				...
				thelabelAnswer.onmouseout = function() {
					var indices = this.value.split("-");
					var str = encrypt[indices[0]][indices[1]];
					var str2 = "";
					if (str.charCodeAt(0) >= 65 && str.charCodeAt(0) <= 90) {
...
function dechipher(index, outerDiv) {
	var crData = map[outerDiv];
	var encrypt = crData.encrypt;
	var divDecipher = crData.solutionDiv;
...
function replaceChar(outerDiv) {
	...
	var encrypt = crData.encrypt;
	var currAnswer = crData.currAnswer;
	var inpBox = crData.solutionDiv.getElementsByTagName("input")[0];
	...
	if ((replacement.charCodeAt(0) >= 65 && replacement.charCodeAt(0) <= 90)
			...
			|| replacement == unknownChar) {
		var lbCiphered = crData.solutionDiv.getElementsByTagName("label")[0];
		...
		var crData1 = map[outerDiv];
		crData1.hoverrevertclass = 'labelcolor1';
		//alert(crData1.hoverrevertclass);
		map[outerDiv] = crData1;
		inpBox.value = "";
	...
	if(crData.solutionSubLineId!=-1){
		var prevSolveBoxLine = document.getElementById(crData.solutionSubLineId);
		prevSolveBoxLine.removeChild(prevSolveBoxLine.lastChild);
	}
